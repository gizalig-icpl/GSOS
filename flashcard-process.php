...
function updateFlashcardStatus($status, array $id){
    global $dbh;
    $ids = implode(",", $id);
    $query = "UPDATE flash_card_master SET status = :status WHERE id IN ($ids)";
    $stmt = $dbh->prepare($query);
...
try {
    ...
    switch ($action) {
        ...
        case 'disable';
            $id = $_POST['ids'];
            updateFlashcardStatus(0, $id);
        break;
        case 'enable';
            $id = $_POST['ids'];
            updateFlashcardStatus(1, $id);
        break;
        default:
            '';
            break;
