<?php
    ...
    if(isset($_GET['id'])){$id=purifier($_GET['id']);}
?>
...
<!-- ============================================================== -->
        ...
        <div class="page-content">
             <!--new breadcrumb-->
                    ...
                    <div class="ps-3 text-end w-50">
                        ...
                        if(isset($_GET['id']))
                        {
                            $sql = "SELECT (SELECT COUNT(*) FROM user_training_detail WHERE status=1 AND training_id=:id)as total_complete,(SELECT COUNT(*) FROM topic  WHERE training_id=:id) as total_topic";
		...
		<!-- <div class="row">
            ...
            <div class="main-title card p-2">
                <h2 class="mb-0 text-success"><?php echo _Training;?></h2>
            </div>
            <div class="content card card-body">
                ...
                <div class="table-responsive mt-3">
                     <table class="table align-middle table-hover">
                       ...
                       <tbody>
                        <?php
                        $sql = "SELECT * FROM training where assign_status=0 order by training_id desc";
                        $stmt = $dbh->prepare($sql);
                        $stmt->execute();
                   ...
                   <video height="100%" width="100%" id="myVideo1" style="display: none;" controls>
                    ...
                    <img src="" id="img_content" style="display: none;" class="w-100" height= "400">
                   </div>
                   </div>
                    ...
                    <div class="table-responsive mt-3">
                     <table class="table align-middle table-hover" id="topictable">
                       ...
                       <tbody class="row_position">
                        <?php
                        $sql = "SELECT * FROM topic where training_id = :id order by position_order";
                        $stmt = $dbh->prepare($sql);
                        $stmt->execute(array(
                            ":id" => $id
                        ));
                        ...
                        ?>
                         <tr id="<?php echo $result[$i]['topic_id'] ?>" onclick="content('<?php echo $result[$i]['path'] ?>','<?php echo $result[$i]['type'] ?>','<?php echo $_SESSION['id'] ?>','<?php echo $id ?>','<?php echo $result[$i]['topic_id'] ?>',1);completed_training('<?php echo $_SESSION['id'] ?>','<?php echo $id ?>','<?php echo $result[$i]['topic_id'] ?>',0);" class="cursor-pointer">
                          <!-- <td><?php echo $i+1 ?></td> -->
                          <td class="text-white">
                            <?php
                            $status=1;
                            ...
                            $user_stmt = $dbh->prepare($user_sql);
                            $user_stmt->bindParam(':user_id',$_SESSION['id'],PDO::PARAM_INT);
                            $user_stmt->bindParam(':training_id',$id,PDO::PARAM_INT);
                            ...
                            if(count($user_result)>0)
                            {
                                ...
                                if($auto_content==1)
                                {
                                    ...
                                    <script>
                                    window.onload = (event) => {
                                        content('<?php echo $result[$i]['path'] ?>','<?php echo $result[$i]['type'] ?>','<?php echo $_SESSION['id'] ?>','<?php echo $id ?>','<?php echo $result[$i]['topic_id'] ?>',1);
                                    };  
                                    </script>
                                    <?php
                                    $auto_content=0;
                            ...
                            ?>
                             <input type="checkbox" title="Not Completed" <?php if($result[$i]['type']!=3){ ?>onclick="sweet('<?php echo $_SESSION['id'] ?>','<?php echo xss_safe($id) ?>','<?php echo xss_safe($result[$i]['topic_id']); ?>',1) "<?php } ?>>
                             <?php
                             }
                             ?>
                           </td>
...
function content(path,type,user_id,training_id,topic_id,status){
    ...
    if(type==1)
    {
        ...
        player.addEventListener('timeupdate', function() {
            if (!player.seeking) {
            if(player.currentTime > maxProgressTime){
                maxProgressTime = player.currentTime;
            }

            supposedCurrentTime = player.currentTime;
            }
        });
        var forward_video='<?php echo config("quiz","forward_video"); ?>';
        if(forward_video==1)
        {
    ...
    if(type==4)
    {
        var player=document.getElementById('myVideo1');
        var pdf = document.getElementById('pdf_content');
        var image = document.getElementById('img_content');
        $('#video_div').css("display", "block");
        $('#myVideo1').css("display", "none");
        $('#pdf_content').css("display", "none");
